-- Fix: accept_budget_by_token silently dropped the vehicle chassis when
-- converting an accepted budget into a transport (vehicle_chassis was never
-- part of the INSERT INTO public.transports column/value list, even though
-- the chassis value is present in body.vehicles[i].chassis). Also, the
-- contract's own observações (body.notes) were being discarded and replaced
-- with a synthetic "Veículo: <description>" string on transports.notes.
--
-- This fix adds vehicle_chassis to the insert and preserves both the
-- per-vehicle description tag and the original observações on notes.
