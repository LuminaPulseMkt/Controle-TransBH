-- Item 16 of the client briefing: client e-mail and address already exist on
-- the contract/document (client_email column, body.client_address), but
-- transports had no equivalent columns, so this data was lost when a budget
-- became a transport, and couldn't be entered on the manual transport form
-- either. Add the two columns and populate them automatically from the
-- accepted budget.
