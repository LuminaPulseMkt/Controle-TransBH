...
create or replace function public.accept_budget_by_token(_token text, _estimated_delivery date default null)
returns jsonb
language plpgsql
security definer
...
exception
  when others then
    raise log 'accept_budget_by_token failed: %', sqlerrm;
    return jsonb_build_object('ok', false, 'error', 'Não foi possível processar a solicitação. Tente novamente.');
end;
$$;

grant execute on function public.accept_budget_by_token(text, date) to anon, authenticated;