-- Item 4 of the client briefing: replace the transport_status enum with the
-- more granular operational statuses requested (aguardando_coleta →
-- coletado_aguardando_embarque → veiculo_patio_aguardando_retirada →
-- finalizado), keeping "cancelled" as a separate terminal status.
--
-- Existing rows are remapped: pending -> aguardando_coleta,
-- in_transit -> coletado_aguardando_embarque (client's choice: the more
-- conservative of the two new sub-stages), delivered -> finalizado,
-- cancelled -> cancelled (unchanged).

CREATE TYPE public.transport_status_new AS ENUM (
  'aguardando_coleta',
...
ALTER TABLE public.transports
  ALTER COLUMN status TYPE public.transport_status_new
  USING (
    CASE status::text
      WHEN 'pending' THEN 'aguardando_coleta'
      WHEN 'in_transit' THEN 'coletado_aguardando_embarque'
      WHEN 'delivered' THEN 'finalizado'
      WHEN 'cancelled' THEN 'cancelled'
    END
  )::public.transport_status_new;

ALTER TABLE public.transports
  ALTER COLUMN status SET DEFAULT 'aguardando_coleta'::public.transport_status_new;

DROP TYPE public.transport_status;
ALTER TYPE public.transport_status_new RENAME TO transport_status;

-- Update the budget-acceptance RPC to create new transports in the new
-- initial status instead of the retired 'pending' value.
